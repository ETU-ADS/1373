...
//¬ывод минимального остновного дерева
void Graph::printMST()
{
    int weight = 0;
    ...
    std::cout << ""<< std::endl;
    std::cout << "¬ес минимального остовного дерева равен: " << weight << std::endl;
    std::cout << "" << std::endl;
}
//
...
void Graph::sort()
{
    ...
    for (int i = 1; i < ribs.getSize(); ++i)
    {
        key = ribs[i];
        j = i - 1;
        while (j >= 0 && ribs[j].w > key.w)//пока текущее ребро больше чем следующее
        {
            ribs[j + 1] = ribs[j]; //переставл€ем
            --j;
        }
        ribs[j + 1] = key;
    }
...
void Graph::kraskal()
{
    int first, second;//раскраски первой и второй вершины
    sort();
    for (int i = 0; i < vertexs.getSize(); ++i)//красим все вершины в разный цвет
        makeSet(i);
    for (int i = 0; mst.getSize() < vertexs.getSize() - 1; ++i)
    {
        first = findSet(ribs[i].start);
        second = findSet(ribs[i].end);
        if (first != second)//если разный цвет, то красим в один
        {
            mst.addBack(ribs[i]);
