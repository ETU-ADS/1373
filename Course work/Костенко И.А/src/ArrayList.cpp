...
void ArrayList<T>::deleteItem(int index)
{
	...
	arr[lastItemIndex - 1] = -1;
	lastItemIndex--;
}

template<typename T>
T ArrayList<T>::find(T index)
{
...
{
	if (typeid(Rib).name() == typeid(T).name()) {
		for (int i = left + 1; i <= right; i++)
		{
			Rib current = arr[i];
			int prevItem = i - 1;
			while (prevItem >= left && arr[prevItem].weight > current.weight)
			{
...
{
	if (typeid(T).name() == typeid(Rib).name())
	{
		arr = new T[count];
		maxSize = count;
	}
	if (typeid(T).name() == typeid(ArrayList<int>).name())
	{
		arr = new T(count*2);
