...
void merge(int* arr, int start, int mid, int end)
{
    ...
    int countl = 0, countr = 0, buff;
    bool gallop = false, offGallop = false;
    bool leftLessRight = false;
    while (indexl < lenl && indexr < lenr)//пока один из массивов не закончится
    {
        if (gallop)
        {
            if (countl)
            {
                ...
                {
                    if (indexl + (2 << (countl - 7)) < lenl)//проверка на выход за предел
                        buff = indexl + (2 << (countl - 7));
                    else if (buff != lenl - 1)//если перепрыгнули то возвращаемся на последний элемент
                        buff = lenl - 1;
                    else
                    {
                        leftLessRight = true;
...
void timSort(int* arr, int size)
{
    ...
    for (int i = 1; i < size; )
    {
        if (arr[i] >= arr[i - 1])
        {
            //добавляем в ран пока отсортировно
